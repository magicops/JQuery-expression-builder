...
function expressionBuilder(selector, options) {
    ...
    function onInput() {
        ...
        var txt = expressionInput, val = txt.val().toString(), cursor = $(this).get(0).selectionStart, lastChar = "", input = lastText.length > val.length ? '\b' : val[cursor - 1], hasSpace = false, _inVariable = false, _inString = false;
        if (isPaste) {
            isPaste = false;
    ...
    function selectVariable(div) {
        var text = expressionInput.val().toString(), start = parseInt($(div).attr('data-start')) + 1, current = parseInt($(div).attr('data-current')), tail = text.substr(current);
        if (tail != '') {
